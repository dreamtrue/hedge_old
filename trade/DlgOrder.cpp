	...
	, m_underComp(new UnderComp)
{
	...
	if(sys.wDay < A50Final){//到最后一天的前一天换合约
		m_expiry.Format(_T("%d%.2d"),sys.wYear,sys.wMonth);
		lifeOfA50 = A50FinalDay(sys.wYear,sys.wMonth + 1) - sys.wDay + 1;
	}
	else{	
		if(sys.wMonth < 12){
			m_expiry.Format(_T("%d%.2d"),sys.wYear,sys.wMonth + 1);
			lifeOfA50 = MonthDays(sys.wYear,sys.wMonth) - sys.wDay
				+ A50FinalDay(sys.wYear,sys.wMonth + 1) + 1;
		}
		else{//年末
			m_expiry.Format(_T("%d%.2d"),sys.wYear + 1,1);
			lifeOfA50 = MonthDays(sys.wYear,sys.wMonth) - sys.wDay
