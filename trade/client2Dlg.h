...
public:
	...
	int setupIfThread(void);
	int setIfTradeSystem(void);
	...
	*/
	long PlaceOrder_hedge(bool whatIf, const std::auto_ptr<Order>& myorder);
	long CClient2Dlg::ModifyOrder_hedge(long id,const std::auto_ptr<Order>& myorder);
	std::map<long,std::vector<rtn>> m_idrtn;
	int m_positionA50;
	afx_msg void OnBnClickedButton10();
	bool tradeEnd;
	double m_step;
	int m_numINI;
	double m_DeltaDatumDiff;
	double m_costINI;
	/*
