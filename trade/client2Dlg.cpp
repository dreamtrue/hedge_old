...
#endif
extern int multiply;
extern int aimOfLadder;
extern long myOrderIdA50;//交易ID
extern DWORD tradeid;//交易线程ID
//同步IF价格写入和读取的临界区
CRITICAL_SECTION g_IfPrice;//临界区
//同步A50价格写入和读取的临界区
CRITICAL_SECTION g_A50Price;
//指数临界区
CRITICAL_SECTION g_index;
//A50trading section
CRITICAL_SECTION tdi_a50;
CRITICAL_SECTION REQUEST_ID_IF;//if请求ID操作临界区
CRITICAL_SECTION ORDER_ID_A50;//a50ID操作临界区
CRITICAL_SECTION avgPriceOfIf;
CRITICAL_SECTION ifDealRtn;//处理成交信息返回临界区
CRITICAL_SECTION ODREF;//if合约报单引用临界区
//事件同步
CWinThread *tradeThread;
extern UINT TradeEntry_00(LPVOID pParam);
extern UINT TradeEntry_01(LPVOID pParam);
int getIndexData(void);
void IfTradeEntry(void);
UINT IfThreadEntry(LPVOID pParam);
UINT IfTradeEntry(LPVOID pParam);
bool isTradeRunning = false;
extern TThostFtdcInstrumentIDType INSTRUMENT_ID;	// if合约代码
extern double A50Index;
extern double HS300Index;
extern double ifBid1,ifAsk1;//买一，卖一价
double a50Bid1,a50Ask1;////买一，卖一价
double a50Bid1Size,a50Ask1Size;
//外盘可用资金余额
double availA50;
//内盘可用资金余额
extern double availIf;
CWinThread *IfThread;
CWinThread *IfTradeThread;
CClient2Dlg *clientDlg;//给其他线程共享界面客户端对象

extern int iRequestID;//命令请求ID统一起来
...
#undef PUT_PROP
	...
	clientDlg = this;
	//初始化临界区
	::InitializeCriticalSection(&g_IfPrice);//临界区
	::InitializeCriticalSection(&g_A50Price);//临界区
	::InitializeCriticalSection(&g_index);//临界区
	::InitializeCriticalSection(&tdi_a50);
	::InitializeCriticalSection(&REQUEST_ID_IF);//if请求ID操作临界区
	::InitializeCriticalSection(&ORDER_ID_A50);
	::InitializeCriticalSection(&avgPriceOfIf);
	::InitializeCriticalSection(&ifDealRtn);
	::InitializeCriticalSection(&ODREF);
}
...
void CClient2Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST1, m_errors);
	DDX_Text(pDX,IDC_EDIT19, m_datumDiff);
	DDX_Text(pDX,IDC_EDIT23, m_step);
	DDX_Text(pDX,IDC_EDIT20, m_numINI);
	DDX_Text(pDX,IDC_EDIT22, m_DeltaDatumDiff);
	DDX_Text(pDX,IDC_EDIT24, m_costINI);
	DDX_Text(pDX,IDC_EDIT25, m_multiply);
	DDX_Text(pDX,IDC_EDIT26, m_aimOfLadder);
	TRACE("m_multiply%d\r\n",m_multiply);
	TRACE("m_aimOfLadder%d\r\n",m_aimOfLadder);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CClient2Dlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON9, &CClient2Dlg::OnBnClickedButton9)
	ON_BN_CLICKED(IDC_BUTTON10, &CClient2Dlg::OnBnClickedButton10)
	/*
	ON_EN_CHANGE(IDC_EDIT19, &CClient2Dlg::OnEnChangeEdit19)
	ON_EN_KILLFOCUS(IDC_EDIT19, &CClient2Dlg::OnKillfocusEdit19)
	ON_EN_CHANGE(IDC_EDIT20, &CClient2Dlg::OnEnChangeEdit20)
	ON_EN_KILLFOCUS(IDC_EDIT20, &CClient2Dlg::OnEnKillfocusEdit20)
	ON_EN_KILLFOCUS(IDC_EDIT24, &CClient2Dlg::OnEnKillfocusEdit24)
	ON_EN_KILLFOCUS(IDC_EDIT22, &CClient2Dlg::OnEnKillfocusEdit22)
	ON_EN_KILLFOCUS(IDC_EDIT23, &CClient2Dlg::OnEnKillfocusEdit23)
	ON_EN_KILLFOCUS(IDC_EDIT23, &CClient2Dlg::OnEnKillfocusEdit25)
	ON_EN_KILLFOCUS(IDC_EDIT23, &CClient2Dlg::OnEnKillfocusEdit26)
	ON_EN_CHANGE(IDC_EDIT23, &CClient2Dlg::OnEnChangeEdit23)
	*/
	ON_BN_CLICKED(IDC_BUTTON11, &CClient2Dlg::OnBnClickedButton11)
	ON_BN_CLICKED(IDC_BUTTON12, &CClient2Dlg::OnBnClickedButton12)
	ON_BN_CLICKED(IDC_BUTTON13, &CClient2Dlg::OnBnClickedButton13)
END_MESSAGE_MAP()
...
void CClient2Dlg::OnConnect()
{
	...
	*/
	...
	if(isfirst){
		...
		CString insID;
		if(sys.wDay < ifFinal){//到最后一天的前一天换合约
			insID.Format(_T("%d%.2d"),sys.wYear,sys.wMonth);
			lifeOfIf = ifFinal - sys.wDay + 1;
		}
		else{
			if(sys.wMonth < 12){
				insID.Format(_T("%d%.2d"),sys.wYear,sys.wMonth + 1);
				lifeOfIf = MonthDays(sys.wYear,sys.wMonth) - sys.wDay
					+ ifFinalDay(sys.wYear,sys.wMonth + 1) + 1;
			}
			else{//年末
				insID.Format(_T("%d%.2d"),sys.wYear + 1,1);
				lifeOfIf = MonthDays(sys.wYear,sys.wMonth) - sys.wDay
		...
		INSTRUMENT_ID[0] = 'I';
		INSTRUMENT_ID[1] = 'F';
		INSTRUMENT_ID[2] = insID.GetAt(2);
		INSTRUMENT_ID[3] = insID.GetAt(3);
		INSTRUMENT_ID[4] = insID.GetAt(4);
		INSTRUMENT_ID[5] = insID.GetAt(5);
		///////////////////////////////////////////////////////////////////
		SetTimer(0,1000,0);//开启定时器，获取指数行情
		setupIfThread();//开启沪深300指数期货行情获取
		OnReqMktDepth();//开启A50期货市场深度数据
		setIfTradeSystem();//IF期货交易系统开启
		m_pClient->reqAccountUpdates(true,"U1032950");//开启帐户信息更新，推送模式
		isfirst = false;
	}
}
...
{
	CAcctUpdatesDlg dlg;
	m_pClient->reqAccountUpdates(false, dlg.getAcctCode());//先要取消账户更新，才能进行下一次更新
...
void CClient2Dlg::CStringReplace(CString &pXml, const CString &cStr, const CString &rStr, CString &pDest)
{
	...
	do {
		...
		if (pFind == NULL)
			strcat_s(pBuiltBuf,sizeof(pBuiltBuf),pBuf) ;
		else {
			lenFound = pFind - pBuf ;
							  ...
							  int clientId, const CString& whyHeld)
{
	...
	//填充符合ID要求的返回信息更其他交易线程使用
	std::map<long,std::vector<rtn>>::iterator iter;
	::EnterCriticalSection(&tdi_a50);
	iter = m_idrtn.find(orderId);
	if(iter != m_idrtn.end()){
		rtn rtn_temp;
		rtn_temp.status = status;
		rtn_temp.filled = filled;
		rtn_temp.remaining = remaining;
		rtn_temp.avgFillPrice = avgFillPrice;
		rtn_temp.permId = permId;
		rtn_temp.parentId = parentId;
		rtn_temp.lastFillPrice = lastFillPrice;
		rtn_temp.clientId = clientId;
		rtn_temp.whyHeld = whyHeld;
		iter->second.push_back(rtn_temp);
	}
	...
	UpdateTrade.SetEvent();//交易线程设置为有信号状态
}
									 ...
									 const CString &currency, const CString &accountName)
{
	...
	if(key == "NetLiquidation" && currency == "USD"){
		double netA50 = atof(val);
		TRACE("净资%s\r\n",val);
	}
	//TRACE("key=%s   val=%s   currency=%s\r\n",key,val,currency);
...

{
	...
	if(contract.symbol == m_dlgOrder->m_symbol && contract.expiry.Left(6) == m_dlgOrder->m_expiry){
		m_positionA50 = position;
		TRACE("A50持仓%d\r\n",position);
		TRACE("unrealizedPNL%f\r\n",unrealizedPNL);
		TRACE("realizedPND%f\r\n",realizedPNL);
	}
}
...
void CClient2Dlg::OnReqMktDepth()
{
	...
	m_dlgOrder->m_id++;
	::LeaveCriticalSection(&ORDER_ID_A50);
	//m_dlgMktDepth->DoModal();//一定要等深度模态对话框关闭后才执行下面的语句，否则是堵塞的
	//m_pClient->cancelMktDepth(m_dlgOrder->m_id - 1);	
}
...
void CClient2Dlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if(nIDEvent == 0){
		CString A50,HS300;
		if(getIndexData() == 0){	
			A50.Format(_T("%.4f"),A50Index);
			HS300.Format(_T("%.4f"),HS300Index);
			SetDlgItemTextA(IDC_EDIT14,A50);
			SetDlgItemTextA(IDC_EDIT15,HS300);
			SetDlgItemTextA(IDC_STATIC01,_T("OK_Index!"));
		}
		else{
			SetDlgItemTextA(IDC_STATIC01,_T("FAIL_Index!"));
		}
	}
...
//开启沪深300指数期货行情
int CClient2Dlg::setupIfThread(void)
{
	IfThread = AfxBeginThread(IfThreadEntry,0,THREAD_PRIORITY_NORMAL,0,0,NULL);
	return 0;
}

//开启IF合约交易系统
int CClient2Dlg::setIfTradeSystem(void)
{
	IfTradeThread = AfxBeginThread(IfTradeEntry,0,THREAD_PRIORITY_NORMAL,0,0,NULL);
	return 0;
}
...
void CClient2Dlg::OnBnClickedButton1()
{
	this->UpdateData();
	// TODO: 在此添加控件通知处理程序代码
	this->GetDlgItem(IDC_EDIT19)->EnableWindow(false);
	this->GetDlgItem(IDC_BUTTON1)->EnableWindow(false);
	this->GetDlgItem(IDC_EDIT20)->EnableWindow(false);
	this->GetDlgItem(IDC_EDIT22)->EnableWindow(false);
	this->GetDlgItem(IDC_EDIT23)->EnableWindow(false);
	this->GetDlgItem(IDC_EDIT24)->EnableWindow(false);
	this->GetDlgItem(IDC_EDIT25)->EnableWindow(false);
	this->GetDlgItem(IDC_EDIT26)->EnableWindow(false);
}

//启动交易系统
void CClient2Dlg::OnBnClickedButton9()
{
	tradeEnd = false;
	if(!(this->GetDlgItem(IDC_BUTTON1)->IsWindowEnabled())){ 
		//启动交易线程
		tradeThread = AfxBeginThread(TradeEntry_00,0,THREAD_PRIORITY_NORMAL,0,0,NULL);
		this->GetDlgItem(IDC_BUTTON9)->EnableWindow(false);
	}
	else{
		CString displayString(_T("Need to set trade initial data!"));
		MessageBox(displayString, _T("SetUp Error"), MB_ICONINFORMATION);
	}
	// TODO: 在此添加控件通知处理程序代码
}

/*
void CClient2Dlg::OnEnChangeEdit19()
{
...
void CClient2Dlg::OnKillfocusEdit19()
{
	// TODO: 在此添加控件通知处理程序代码
	this->UpdateData();
}
*/
//自定义的placeOrder
long CClient2Dlg::PlaceOrder_hedge(bool whatIf, const std::auto_ptr<Order>& myorder)
{
	Order& order_hedge = *myorder.get();//不做任何额外的处理,因为ExtOrd对话框就是额外加参数的,可以不用管
	::EnterCriticalSection(&ORDER_ID_A50);
	// save old and set new value of whatIf attribute
	bool savedWhatIf = order_hedge.whatIf;
	order_hedge.whatIf = whatIf;
	long rid = m_dlgOrder->m_id;//返回的ID
	m_pClient->placeOrder( m_dlgOrder->m_id, m_dlgOrder->getContract(), order_hedge);
	m_dlgOrder->m_id++;
	::LeaveCriticalSection(&ORDER_ID_A50);
	// restore whatIf attribute
	order_hedge.whatIf = savedWhatIf;
	return rid;
}

long CClient2Dlg::ModifyOrder_hedge(long id,const std::auto_ptr<Order>& myorder)
{
	Order& order_hedge = *myorder.get();
	//if(::GetCurrentThreadId() == tradeid){//通过线程号来识别指令发出方
	//	myOrderIdA50 = id;
	//}
	m_pClient->placeOrder( id, m_dlgOrder->getContract(), order_hedge);
	return id;
}
...
void CClient2Dlg::OnBnClickedButton10()
{
	...
	this->GetDlgItem(IDC_EDIT19)->EnableWindow(true);
	//IDC_EDIT20和IDC_EDIT24是用来做初启动时的初始化的，对于暂停这样的情况，持仓数据仍然保存，则不可以再一次初始化，故设置为非激活状态
	this->GetDlgItem(IDC_EDIT22)->EnableWindow(true);
	this->GetDlgItem(IDC_EDIT23)->EnableWindow(true);
	this->GetDlgItem(IDC_EDIT24)->EnableWindow(true);
	this->GetDlgItem(IDC_EDIT20)->EnableWindow(true);
	this->GetDlgItem(IDC_EDIT19)->EnableWindow(true);
	this->GetDlgItem(IDC_EDIT25)->EnableWindow(true);
	this->GetDlgItem(IDC_EDIT26)->EnableWindow(true);
}

/*
void CClient2Dlg::OnEnChangeEdit20()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 __super::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CClient2Dlg::OnEnKillfocusEdit20()
{
	// TODO: 在此添加控件通知处理程序代码
	this->UpdateData();
}
...
void CClient2Dlg::OnEnKillfocusEdit24()
{
	// TODO: 在此添加控件通知处理程序代码
	this->UpdateData();
}
...
void CClient2Dlg::OnEnKillfocusEdit22()
{
	// TODO: 在此添加控件通知处理程序代码
	this->UpdateData();
}
...
void CClient2Dlg::OnEnKillfocusEdit23()
{
	// TODO: 在此添加控件通知处理程序代码
	this->UpdateData();
}
void CClient2Dlg::OnEnKillfocusEdit25()
{
	// TODO: 在此添加控件通知处理程序代码
	this->UpdateData();
}
void CClient2Dlg::OnEnKillfocusEdit26()
{
	// TODO: 在此添加控件通知处理程序代码
	this->UpdateData();
}

void CClient2Dlg::OnEnChangeEdit23()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 __super::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CClient2Dlg::OnBnClickedButton11()
{
	// TODO: 在此添加控件通知处理程序代码
...
int CClient2Dlg::SendMsg(LPCTSTR lpMsg)
{
	TRACE(lpMsg);
	//this->m_WXin.SendMsg("wxid_wv1so37or1ya21",lpMsg);
	this->m_WXin.SendMsg("dreamtrue_tang",lpMsg);
	this->m_WXin.SendMsg("hanqiong9347",lpMsg);
	return 0;
}


void CClient2Dlg::OnBnClickedButton12()
{
	// TODO: 在此添加控件通知处理程序代码
	stop = true;
	this->GetDlgItem(IDC_BUTTON12)->EnableWindow(false);
	this->GetDlgItem(IDC_BUTTON13)->EnableWindow(true);
}


void CClient2Dlg::OnBnClickedButton13()
{
	// TODO: 在此添加控件通知处理程序代码
	stop = false;
	this->GetDlgItem(IDC_BUTTON13)->EnableWindow(false);
	this->GetDlgItem(IDC_BUTTON12)->EnableWindow(true);
}
